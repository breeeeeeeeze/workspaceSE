...
public class ChatClientFrame extends JFrame {
	...
	public ChatClientFrame() throws Exception {
		...
		chatList.setModel(new AbstractListModel() {
			String[] values = new String[] {"KIM", "PARK", "LEE"};
			public int getSize() {
	...
	public class ClientClientThread extends Thread {
		...
		public ClientClientThread() throws Exception {
			this.socket = new Socket("192.168.15.15", 8888);
		...
		public void run() {
			try {
				while (true) {
					System.out.println("A.ClientClientThread:서버로부터오는 데이타를 읽기위해 쓰레드 무한대기");
					...
					if (chatArray[0].equals(ChatProtocol.PLAIN_MSG)) {
						System.out.println("B.ClientClientThread:서버로부터 읽은 PLAIN_MSG 데이타를 클라이언트 채팅창에 보여준다.");
