		...
		*****************************************************************/
		...
		ParentFirstConcreteClass childConcreteClass = new ParentFirstConcreteClass() {
			...
			public void local_inner_outer_access() {
				System.out
						.println("ParentFirstConcreteClass를 상속받는 이름없는 로컬 내부클래스에서 재정의한 local_inner_outer_access()");
			}

		};
		childConcreteClass.local_inner_outer_access();

		ParentSecondAbstractClass childAbstratClass = new ParentSecondAbstractClass() {
		...
		childAbstratClass.local_inner_outer_access();
		
		ParentThirdInterface childThirdInterface = new ParentThirdInterface() {
			
			@Override
			public void local_inner_outer_access() {
				System.out.println("ParentThirdInterface를 상속받은(구현한) 이름없는 클래스에서 재정의(구현) local_inner_outer_access");
