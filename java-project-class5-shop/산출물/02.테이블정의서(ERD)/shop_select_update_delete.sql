...
-- 로그인한 멤버(guard1)의 카트 1개 아이템 수량 변경 (수정은 다양한 방법으로 가능하다)
update cart set cart_qty = 3 where cart_no = 8; -- pk조건이라 로그인하지 않아도 가능(?)

-- 로그인한 멤버(guard1)의 카트에 존재하는 제품의 수(제품 존재 여부 판단)
...
select * from orders o join order_item oi on o.o_no=oi.o_no  join  product p on oi.p_no=p.p_no where o.userid='guard1' and o.o_no = 2;
select * from orders where userid='guard1';

/****************order********************/
-- 1. 멤버 한 사람의 주문(주문아이템 + 제품정보) 전체 목록(guard1)
select * from orders where userid = 'guard1';
...
-- 2. 멤버 한 사람의 주문(주문아이템 + 제품정보) 한 개 상세 목록(guard1)
select * 
    from orders o 
    join order_item oi 
    on o.o_no = oi.o_no 
